...
func (a Actor) PreCommitSector(rt Runtime, params *SectorPreCommitInfo) *adt.EmptyValue {
	...
	newlyVestedAmount := rt.State().Transaction(&st, func() interface{} {
		...
		// Technically, this could be a state error. However, it's almost certainly an invalid argument error.
		// We can fix this when we fix https://github.com/filecoin-project/specs-actors/issues/597.
		err := st.AllocateSectorNumber(store, params.SectorNumber)
		builtin.RequireNoErr(rt, err, exitcode.ErrIllegalArgument, "failed to allocate sector id %d", params.SectorNumber)
...
func (a Actor) CompactSectorNumbers(rt Runtime, params *CompactSectorNumbersParams) *adt.EmptyValue {
	...
	rt.State().Transaction(&st, func() interface{} {
		...
		// TODO: treat this as an illegal argument for now as the caller
		// may have exceeded the max bitfield size.
		// https://github.com/filecoin-project/specs-actors/issues/597
		builtin.RequireNoErr(rt, err, exitcode.ErrIllegalArgument, "failed to mask sector numbers")
		return nil
	})
	return nil
}
